...
**Addendum (Task 1.1 implementation) — temporary `POST /jobs/` run-create:** the result-ingest endpoint is `POST /jobs/{run_id}/result`, so it needs a `run_id` to already exist. Because 1.1 lands *before* dispatch (1.2), 1.1 ships a **temporary** `POST /jobs/` that mints an emptyish in-progress run (`success=NULL`) for a given `thing_id`. **Task 1.2's dispatcher removes it**: dispatch itself creates the in-progress run when it hands out the top job and returns the `run_id` in its response to the worker (one fewer round-trip). This is only a single-worker stand-in and stays consistent with "creating the run at assignment for the *multi-worker* case is 4.x" above.
