...
2. **DB + API** — the frozen V4 SQLModel tables `Thing`/`Rel`/`Run` and their views (`ThingRead`, `ThingWithRelated`, `RunRead`, `ThingAdd`, `ThingPatch`, `ClaimRequest`, `JobClaim`, `RunResultIn`). See "V4 work" for the schema-freeze rules.
...
- Stage-1 ingest (`POST /jobs/{run_id}/result`) auto-creates a `type='channel'` thing per distinct uploader and links it with `channel_playlist`/`channel_video` `rel` edges, so videos are reachable by channel even without an explicit channel playlist.
