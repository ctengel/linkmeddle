...
> **Amendment (4.1): sub-container identity, one run per pull, hybrid scheduling, duplicate convergence.** Implementation of the fan-out core (PR #190) settled five mechanics this document did not anticipate; all are DML/logic only — the frozen DDL is untouched.
> 1. **Sub-container members are URL-keyed.** A channel's tabs (Videos/Shorts/Live) all share the channel's yt-dlp `id` (#46/#183), so a member with `container=True` is stored with `native_id = NULL` (the id demoted to a soft `attrs.channel_id` hint) and identified by URL — the native key stays the identity for leaves and for a *directly-pulled* canonical container. `_apply_backfill` never converges a channel away on an identity clash (a self-pulling tab cannot delete its channel).
> 2. **One yt-dlp call → one `run`.** An inlined sub-container creates no run of its own; the single run's `entries_hash` is the **subtree** hash (every descendant node, URL-first container keys), so a change anywhere below keeps the parent's §4.4 backoff hot. Consequently a parent-fed thing may carry `last_success_dt` with an empty run history.
> 3. **Hybrid (parent-fed) scheduling.** An inlined sub-container is rescheduled to `try_on = parent.try_on + SAFETY_MARGIN_DAYS` (7d): the parent's next pull normally re-feeds it (no extra site call), and it self-pulls only if the parent goes quiet past the margin. A user permafail ack (`try_on = NULL`, §2.5) is never overwritten by the re-feed. When a parent-fed container does self-pull, its backoff restarts from first-run behavior (no run history).
> 4. **Duplicate rows converge by merge.** Two rows discovered for one real thing (url-first vs native-key-first — the [A8] pre-canonicalization gap) merge when a pull carries both keys: rel/run FKs re-point to the survivor, its still-NULL state/display fields fill from the loser, the loser row is deleted. `try_on` is never carried (a NULL survivor `try_on` means acquired/acked); a carried `best_oi` nulls the survivor's `try_on`.
> 5. **Id-only uploader channels (#160).** An uploader exposed by ID alone (vk, twitchvod) gets a url-less, native-id-keyed channel `thing` (pure graph node; dispatch skips unrunnable containers) that merges into the real channel when it is later pulled. The V3 `pseudo_channel` parity promised in §2.1 thus covers URL-less uploaders too.
...
- **Display columns (`title`, `channel`, `thumbnail_url`) are denormalized onto `thing`** `[A2-A]`, updated opportunistically on each run. The exact set may be tuned at implementation; the door to a JSON-only approach stays open if it bloats, but A is the chosen default. This is also what resolves cross-metadata references (#137 / `[G1]`): a video discovered only inside a playlist gets a **stub `thing`** created with whatever we know (title/channel pulled out of the parent's `run.data_json`, plus the inherited `bucket` and any propagated `attrs.cookies`/`attrs.lpm_lib` hints — §4.1), so we never need a "this video's metadata lives at playlist X item Y" pointer. Fan-out likewise auto-creates a channel `thing` (a container tagged `attrs.kind='channel'`) per **distinct uploader** — the playlist's *and* each video's — so videos stay reachable by channel even outside an explicit channel playlist (full parity with V3's per-video `pseudo_channel`). The playlist's channel gets a `channel=True` rel edge to the playlist; each video's uploader channel gets a `channel=True` rel edge to that video (a video whose uploader matches the playlist's reuses the one channel node). This is the V4 form of V3's `pseudo_channel` auto-creation; the `channel=True` flag on the edge (not a separate node type) is the marker `[A11]`. 4.0 only creates these channel nodes/edges; channel *display/navigation* in the FE is 4.x (#120).
...
**The edge carries no free-text `type`** — its one distinction (`channel`) is the boolean above. 4.x may add a **`same`** edge to link two things that are the *same media* when URL canonicalization (`[A8]`) can't merge them (e.g. two distinct URLs that resolve to one video); it returns as an **additive** nullable column or side table, never by reviving a free-text `type`.
...
- **A user-*added* thing defaults to `human_rating = 0` (C).** When a human adds a thing (§3.2 add-by-URL), the conservative default is **C ("track it, don't acquire yet")**, overridable to **B (+1)** or **A (+2)** at add time (D/F are not add-time options — you don't add something in order to suppress it). The propagation makes this do the right thing: since a video's machine rating is the MAX of its parent playlists' human ratings, adding a *playlist* at C makes its members assess as C — tracked-but-not-acquired until a human bumps them; adding at B makes members assess as B and download; A adds replication. *(The earlier draft defaulted adds to B/acquire; 4.0 ships the conservative C default — the human explicitly bumps to B to start downloading.)*
...
| `lmfe/models.py` — `ThingBase` | **Adapt** | already prefigures V4; add rating/`try_on`/timestamps; `type` → `container` boolean |
...
- The **add-a-thing-by-URL action** — the primary human entry point. The user supplies a URL (optionally a grade); the `thing` is created with `try_on = today` (creation default, §2.5) and **default `human_rating = 0` / C**, overridable to B or A (§2.4). This is the V4 form of V3's "add a playlist" (`POST /schedules/`), minus `freq_days`. **URL-classify is deferred to 4.x** (implementation decision, Task 0.3): in 4.0 the add just records the URL with `container` defaulting to **NULL (unknown — the user never declares a type, #153)**; `extractor_key`, `native_id`, and the real `container` classification are filled in by the worker on result-ingest (§3.3, Phase 1) when the job actually runs — so no yt-dlp `suitable()`/classify step at add time.
...
*Things*
- `POST /things/` — **add by URL** (the human entry point): record the URL, **required `bucket`** (OI storage bucket, §2.1 — no server default; the frontend supplies its `OBJIDX_BUCKET_DEFAULT`), `container` default NULL (unknown — the user never declares a type, #153; classify deferred to 4.x — §3.2), default `human_rating=0`/C (override B/A), `try_on=today` (§2.4, §3.2). **Optional** `cookies` suggestion and `lpm_lib` tag may also be supplied — both stored as soft hints in `attrs` (§2.1), in contrast to the required `bucket`. Idempotent on `url` (returns the existing thing, with its original `bucket` unchanged) — closes the #142 race.
...
- `PATCH /things/{id}` — set `human_rating` (including D/F = delete intent), permanent-failure ack (`try_on=NULL`), and edit the `attrs.cookies`/`attrs.lpm_lib` **soft hints** (V3 PATCH-schedule parity; an explicit `null` clears a hint) beyond their add-time/fan-out setting (Task 1.3b). *(Title backfill (#147) is the worker's job on result-ingest — Task 1.1, not this endpoint. No `DELETE` verb in 4.0 — normal deletion is via the D/F rating, and the 4.x admin hard-remove can be added then.)*
...
- `POST /jobs/claim` — **prioritized dispatch** (§4.5): returns the single highest-priority due job (`{run_id, thing, download, cookies}`, where `download` is a boolean — True only for a video assessing ≥ B, §4.2 — and `cookies` is the server's per-job cookies suggestion — §4.7) via the §4.2 predicate + `SKIP LOCKED`. The worker runs one code path for every job: it always extracts as flat as possible (`extract_flat='in_playlist'`, a no-op on a single video) and shapes its result body from what yt-dlp returned (playlist fan-out vs single `video`); `download` is the only knob (acquire media + set `best_oi`, or metadata only). 4.x accepts worker self-selection filters (`type`/`extractor`/`site`/`backend`).
- `POST /jobs/{run_id}/result` — **report a completed job** (rewrite of V3's `POST /playlist-run`, now run-scoped): **the worker posts here** after running yt-dlp — one metadata-push path for all job kinds, rather than the in-plugin postprocessor POSTing. Stage 1 fan-out ingest (upsert `thing`+`rel`+`run`, `entries_hash`, `playlist_count`, `thing.modified`), or a Stage 2 video outcome where the **metadata-only enrich and the download share one metadata-ingest path**: both forward the full single-video `video` VidFull, which the server uses to enrich the stub identically — NULL-backfill display + identity (#147) and fan out the uploader's channel (`channel=True` rel edge) the flat pull omitted — then mark `last_success_dt`. The only divergence is the outcome: a `download` sets `best_oi` + `try_on = NULL` (acquired); a metadata-only video run keeps the `attrs.info_json` load-info hint + Fibonacci backoff (still pending, **no `best_oi`**). This matters for the **stub → download** path (a video machine-rated A/B never sees a metadata-only or human step, so the download's full extract is the only metadata it gets). The server computes the backoff `try_on`.
...
**Stage 1 — Learn.** A playlist pull fetches metadata only (no downloads), upserts a `thing` for every discovered member (stub things get denormalized `title`/`channel` from the pull, **inherit the parent playlist's `bucket`**, and inherit the parent's soft hints — `attrs.cookies` when the parent has it, `attrs.lpm_lib` only if present — §2.1), inserts `rel` edges, and records a `run`. It also creates a channel `thing` (a container tagged `attrs.kind='channel'`) per distinct uploader (the playlist's *and* each video's), linking it to the playlist and to each of its videos via a `channel=True` rel edge (V4's `pseudo_channel` replacement, §2.1). The pull is **as flat as possible** (`extract_flat='in_playlist'` in `run_bknd`): minimal calls to the video site — just enough for each entry's id/url/title — with any richer fields the site volunteers being a bonus. Deeper per-video extraction is **not** done inline; under-described videos are enriched on demand by the **metadata-only video job** below. Whether a stub is **metadata-complete** is decided **API-side, from the extracted denormalized fields** (`xform.enough_to_rate`, called by the ingest endpoint — never by peeking at raw yt-dlp `info_json`, which only the worker touches): 4.0 requires the **full identity set — `title`, `url`, `native_id`, `extractor_key`, and `channel` — all present** for a human to make the acquire/skip call (richer metadata arrives for free if the video is later downloaded; the field set is tunable per-site `[C4]`, **left to live testing** — an earlier draft gated on `title` alone, 4.0 ships the stricter full-identity gate). A metadata-complete stub gets `last_success_dt = today`; a title-less stub is left `last_success_dt = NULL` so it is enriched by a separately-dispatched **per-video metadata-only job** (§4.2 metadata-only branch). *(This supersedes the earlier "enrich inline during the playlist pull / no per-video metadata job" decision: rather than blocking the playlist-learn process on deep per-video extraction, the pull stays cheap/flat and under-described videos are enriched by an independently-prioritized metadata-only run — still **not** the rejected standalone metadata *stage*, just a third eligibility branch on the one dispatch ordering, gated to C-band videos that lack human-decision metadata. It dispatches with `download=False`, exactly like a container pull — there is no distinct worker "meta" action, just the single flat-extract path with the media download switched off.)*
...
**Stage 2 — Acquire (or enrich).** Not a separate timed pass — just the dispatch endpoint (§4.5) returning a *video* job when one outranks the available playlist jobs. Two video outcomes share the one ordering; both run the worker's single flat-extract path (flat is a no-op on a single video, so each gets a full extract), differing only in the `download` flag:
- **download** (`download=True`) — a video whose effective rating is ≥ B, not yet acquired, and `try_on`-due. The runner downloads it via yt-dlp (media to OI via `ObjIdxUploadPP` or worker-side upload, **into the bucket carried on the dispatched `thing.bucket`** — §2.1, the V4 replacement for V3's `schedule.oi_bucket`) — a full extraction, so it pushes the same single-video `video` metadata a metadata-only run would (the server enriches display + channel identically, §3.3) **alongside** the media — then **pushes to PLAPI** (`POST /jobs/{run_id}/result`), which sets `best_oi` and `try_on = NULL`. A stub machine-rated A/B that skips straight to download is thus still fully described by this one extract.
- **metadata-only** (`download=False`) — a C-band video (effective rating in [C, B)) the flat pull under-described (`last_success_dt IS NULL`). The runner does a single-video extract — metadata only, no media, no OI — and pushes a single `VidFull`; the result enriches the stub, **fans out the uploader the flat pull omitted** (channel `thing` + `channel=True` rel edge), sets `last_success_dt = today` (human-decision metadata now in hand, set unconditionally on success so a still-bare video never loops back to another metadata-only job), and backs `try_on` off — **no `best_oi`** (not acquired). It then sits in Phase II until a human rates it (a raise-to-eligible PATCH pulls `try_on` to today, §2.5, and the download branch acquires it).

### 4.2 Job selection predicate `[B2-B, C7]`

This predicate lives **inside the API dispatch endpoint** (§4.5), which owns prioritization; the runner never queries `thing` directly — it asks the endpoint for the next job. Human rating always wins; anything assessing below the **C band** (r < −0.5, i.e. grades D/F) is never touched. Thresholds use the **grade bands** (§2.4), not bare integers, and an unrated thing assesses as **C (0.0)** — `COALESCE(human_rating, machine_rating, 0.0)` — so the effective-rating expression must default NULL to 0.0 or every comparison fails. Rather than separate passes, a single ordering spans all job kinds (playlist / video), so the endpoint returns whichever is highest-priority right now; the three predicates below are branches of that one ordering. The dispatched **`download` flag** is True iff the video-download branch matched (a leaf video ≥ B); the other two branches — a container/unknown pull and a C-band video metadata-only enrich — both dispatch with `download=False`. The worker runs the same single flat-extract path for all three; `download` is the only knob (acquire media + set `best_oi`, or metadata only).

```sql
-- Stage 1 (container/unknown metadata pull) -> download=False: grade >= C band
...
-- Stage 2 (video download) -> download=True: grade >= B band
...
-- Stage 2 (video metadata-only) -> download=False: C band, no human-decision metadata yet
WHERE container IS FALSE              -- a leaf video
...
> **Implementor note — `SELECT ... FOR UPDATE SKIP LOCKED`:** In one transaction, **the dispatch endpoint** runs its selection query (§4.2) with `FOR UPDATE SKIP LOCKED`. `FOR UPDATE` takes a row-level lock on each selected row; `SKIP LOCKED` tells Postgres to silently *skip* any row already locked by another transaction instead of blocking on it. The result: two concurrent dispatch calls never hand out the same `thing` — each gets disjoint currently-unlocked rows. The endpoint records the claim by **creating the `run` row at claim time** (`success = NULL`, `worker` set, `starttime` = the assignment instant — see §2.3) and returns its `run_id` to the worker (the `{run_id, thing, download}` of §3.3); it then commits to release the locks. *(Reconciled in the Task 1.2 build: 4.0 creates the run at claim — superseding the earlier "4.0 returns the job; the runner records the run" split — so the result-ingest endpoint always has a `run_id`; what stays 4.x is worker self-selection and multi-worker coordination.)* This is the idiomatic Postgres work-queue pattern, costs a few words of SQL, fixes the real `POST /playlists/` race (#142) at the DB level, and makes the single-worker 4.0 design forward-compatible with multi-worker 4.x at near-zero cost.
...
| 1.3c | **Channel fan-out** (do **before/with 1.3**) `[A11]`. **No schema change** (the `rel.channel` boolean already exists). Stage-1 ingest (1.1 retrofit) auto-creates a channel `thing` (a container tagged `attrs.kind='channel'`) per **distinct uploader** (playlist's + each video's) + a `channel=True` rel edge to the playlist and to each video; same-uploader videos share one channel node. V4 form of V3 `pseudo_channel`. 4.0 builds the graph only; channel FE is 4.x | **S** | #120, #46 | `rel.channel=True` is the marker; reachable-by-channel |
