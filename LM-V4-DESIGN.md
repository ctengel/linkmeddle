...
- **`best_oi` is denormalized** `[A5-A]` and set by the worker on a successful download. **In 4.0 nothing else touches it** — rating D/F does not delete media, so `best_oi` keeps pointing at the live object. The **4.x OI scrubber** is what deletes D/F media and repoints `best_oi` to the tombstone (never cleared to NULL). OI does not change on its own, so the worker setting it at download time is all 4.0 needs.
...
CREATE TABLE rel (
  ...
  type       text NOT NULL,         -- 'playlist_video' | 'channel_playlist' | ...; 4.x may add 'same' (two things are the same media)
...
CREATE TABLE run (
  ...
  input_json jsonb,                 -- parameters passed in (oi_bucket, cookies flag, ...)
...
A single nullable date encodes all scheduling intent (replacing V3's fixed `freq_days`):
...
| `lmdb/models.py` — `PlaylistSched` | **Replace** | folded into `thing.try_on` (no `freq_days`) |
| `lmdb/models.py` — `PlaylistStats` | **Replace** | becomes the `run` table |
| `lmdb/xform.py` — Fibonacci backoff (`FIB`, `next_fib`, `rec_adjust_freq`, `add_new_run`) | **Reuse** | `try_on` interval computation (§4.4) |
...
- `POST /jobs/claim` — **prioritized dispatch** (§4.5): returns the single highest-priority due job (`{run_id, thing, action}`) via the §4.2 predicate + `SKIP LOCKED`. 4.x accepts worker self-selection filters (`type`/`extractor`/`site`/`backend`).
...
Reuse the existing Fibonacci logic (`FIB = [1,2,3,5,8,13,21,34]` days, `next_fib`, `rec_adjust_freq`, `add_new_run`). **No "hold for first 3 runs" guard** (#149): that guard was a workaround for bug #97, whose real fix is fan-out itself. Instead:
...
> **Implementor note — `SELECT ... FOR UPDATE SKIP LOCKED`:** In one transaction, **the dispatch endpoint** runs its selection query (§4.2) with `FOR UPDATE SKIP LOCKED`. `FOR UPDATE` takes a row-level lock on each selected row; `SKIP LOCKED` tells Postgres to silently *skip* any row already locked by another transaction instead of blocking on it. The result: two concurrent dispatch calls never hand out the same `thing` — each gets disjoint currently-unlocked rows. The endpoint records the claim (in 4.0: returns the job; in 4.x: also writes an **assigned** `run` row — `success = NULL`, `worker` set, `starttime` = the assignment instant — see §2.3) and commits to release the locks. This is the idiomatic Postgres work-queue pattern, costs a few words of SQL, fixes the real `POST /playlists/` race (#142) at the DB level, and makes the single-worker 4.0 design forward-compatible with multi-worker 4.x at near-zero cost.

- **Job assignment is deliberately thin in 4.0** and grows in 4.x: writing the `run` at assignment time, **worker self-selection** (the worker tells the dispatch endpoint which job kinds it will accept — by `type` playlist/video, `extractor_key`, `site`, or `backend` — and the endpoint returns only matching jobs), and coordinating multiple workers via the `worker` column. The schema already supports all of it (those are existing `thing` columns + `run.worker`) — no migration (Appendix A).
...
**Critical path:** 0.1 → 0.2 → 0.3 → 1.1 → 1.2 → 1.3 → 1.4 → 2.2 → 2.3, then 3.x. Rating CRUD (2.1) should be pulled forward and built alongside Phase 1 so end-to-end manual testing is possible before machine rating (2.2) lands.
...
- **Multiple simultaneous workers** (#27 partial / #103) — the `SKIP LOCKED` claim query is already built for this.
- **Richer job assignment** `[D2]` — the dispatch endpoint writes an **assigned** `run` row at hand-out time (`success=NULL`, `worker` set, `starttime` = assignment instant), plus **worker self-selection** and multi-worker coordination via the `worker` column. *Worker self-selection:* a worker declares which job kinds it is willing to take — filtering by `type` (playlist vs video), `extractor_key`, `site`, or `backend` — and the dispatch endpoint hands back only the highest-priority job matching that filter. Useful for heterogeneous workers (e.g. one box holds the cookies/IP for a given site, or a worker is dedicated to video downloads vs playlist pulls). All filter dimensions are existing `thing` columns, so this is pure endpoint/runner logic — no migration. 4.0 keeps assignment thin: hand out the single top job, runner records the run when work begins.
...
- **Distributed/parallel worker architecture** (#27 full, #103) beyond the 4.x multi-worker step.
