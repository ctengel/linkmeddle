...
- `POST /jobs/claim` — **prioritized dispatch** (§4.5): returns the single highest-priority due job (`{run_id, thing, action, cookies}`, where `action` is `pull` | `download` | `meta` — §4.2 — and `cookies` is the server's per-job cookies suggestion — §4.7) via the §4.2 predicate + `SKIP LOCKED`. 4.x accepts worker self-selection filters (`type`/`extractor`/`site`/`backend`).
- `POST /jobs/{run_id}/result` — **report a completed job** (rewrite of V3's `POST /playlist-run`, now run-scoped): **the worker posts here** after running yt-dlp — one metadata-push path for all job kinds, rather than the in-plugin postprocessor POSTing. Stage 1 fan-out ingest (upsert `thing`+`rel`+`run`, `entries_hash`, `playlist_count`, `thing.modified`), or a Stage 2 video outcome where **`meta` and `download` share one metadata-ingest path**: both forward the full single-video `video` VidFull, which the server uses to enrich the stub identically — NULL-backfill display + identity (#147) and fan out the uploader's channel (`channel_video` rel) the flat pull omitted — then mark `last_success_dt`. The only divergence is the outcome: `download` sets `best_oi` + `try_on = NULL` (acquired); `meta` keeps the `attrs.info_json` load-info hint + Fibonacci backoff (still pending, **no `best_oi`**). This matters for the **stub → download** path (a video machine-rated A/B never sees `meta` or a human step, so the download's full extract is the only metadata it gets). The server computes the backoff `try_on`.
...
**Stage 2 — Acquire (or enrich).** Not a separate timed pass — just the dispatch endpoint (§4.5) returning a *video* job when one outranks the available playlist jobs. Two video actions share the one ordering:
- **`download`** — a video whose effective rating is ≥ B, not yet acquired, and `try_on`-due. The runner downloads it via yt-dlp (`maybe_playlist=False`; media to OI via `ObjIdxUploadPP` or worker-side upload, **into the bucket carried on the dispatched `thing.bucket`** — §2.1, the V4 replacement for V3's `schedule.oi_bucket`) — a full extraction, so it pushes the same single-video `video` metadata a `meta` job would (the server enriches display + channel identically, §3.3) **alongside** the media — then **pushes to PLAPI** (`POST /jobs/{run_id}/result`), which sets `best_oi` and `try_on = NULL`. A stub machine-rated A/B that skips straight to download is thus still fully described by this one extract.
- **`meta`** — a C-band video (effective rating in [C, B)) the flat pull under-described (`last_success_dt IS NULL`). The runner does a **full single-video** extract (not flat) — metadata only, no media, no OI — and pushes a single `VidFull`; the result enriches the stub, **fans out the uploader the flat pull omitted** (channel `thing` + `channel_video` rel), sets `last_success_dt = today` (human-decision metadata now in hand, set unconditionally on success so a still-bare video never loops back to another meta job), and backs `try_on` off — **no `best_oi`** (not acquired). It then sits in Phase II until a human rates it (a raise-to-eligible PATCH pulls `try_on` to today, §2.5, and the `download` branch acquires it).
